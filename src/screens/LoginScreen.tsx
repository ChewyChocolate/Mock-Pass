...
export default function LoginScreen({ onNavigate }: BaseScreenProps) {
  ...
  const { signIn, signUp, isConfigured, isLoading, error, clearError } = useAuth();
  ...
  const isDisabled = submitting || isLoading || !isConfigured;
  const submitLabel = mode === 'sign-in' ? 'AUTHENTICATE' : 'CREATE ACCOUNT';
